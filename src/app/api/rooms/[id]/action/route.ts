...
import { assertCanStartHand, assertRoomAccess } from "@/lib/room-access";
...
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    ...
    if (parsed.data.action === "start") {
      await assertCanStartHand(id, authResult);
    ...
    const status = /access|seated|not found/i.test(message) ? 403 : 400;
